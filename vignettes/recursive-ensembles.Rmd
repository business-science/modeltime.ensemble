...
Here's an example of a panel forecast that uses a recursive ensemble with a Elastic Net and MARS model.
...
1. The first model is an Elastic Net (GLMNET) model 
2. The second model is a MARS model

The elastic net applies penalty to the lagged regressors preventing bad lags from dominating the results. This can show an improvement versus a standard Linear Regression. 
...
model_fit_mars <- mars("regression", num_terms = 7) %>%
    set_engine("earth", endspan = 24) %>%
    fit(value ~ ., data = train_data)
```

The next step is to create an ensemble of type mean (in which the predictions of the two models will be averaged) and right after that we use the `recursive()` function to create the recursive model. Unlike the previous example:
